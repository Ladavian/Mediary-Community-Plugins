...
每次计划动作会返回标准 `notice` 和详细 `report`，由 Mediary 展示结果；最近一次成功执行也会保存到插件的数据视图。插件还会通过 Mediary 系统通知提示发现可更新容器、自动更新与备份结果，以及执行失败。通知默认开启；“无更新时也通知”默认关闭，避免产生无意义的周期性提醒。插件不会访问 Mediary 内部数据库或宿主凭据。
...
```bash
./build.sh 1.0.6 x86_64-unknown-linux-gnu
```

会生成 `dockercopilot-helper-1.0.6-linux-amd64.tar.gz`。压缩包根目录直接包含 `plugin` 和 `plugin.json`，可按 Mediary 插件开发指南安装。GitHub Release 同时提供 Linux AMD64 与 ARM64 安装包。
...
- **通知**：默认发送发现更新、自动更新、备份和失败通知；可关闭通知，或开启“无更新时也通知”。
